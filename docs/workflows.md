...
결과의 `session.id`를 이후 모든 호출에 사용한다. `write_enabled`, `sandbox_backend`, `code_mode_configured`, `native_aside_enabled`를 확인한다. `sandbox_backend`는 `seatbelt`(macOS 기본), `docker`, `none` 중 하나다. `code_mode_configured: true`는 백엔드가 설정되어 있다는 뜻이지 그 백엔드가 지금 정상이라는 뜻이 아니다. 도구 실패를 다른 호스트 실행기로 우회하지 않는다.
...
`job_cancel`은 중단을 요청한다. 이후 상태를 다시 확인한다. 이미 반영된 파일 수정이나 브라우저 행동은 남아 있을 수 있다. 서버가 정상 종료하면 활성 작업을 취소하고 기록을 닫는다. 강제 종료 후 남은 `running` 기록은 재시작 때 `interrupted`로 바뀌며 자동 재실행하지 않는다. Docker 백엔드에서 강제 종료가 남긴 컨테이너는 운영자가 확인·정리해야 한다. Seatbelt 백엔드에는 정리할 데몬 측 상태가 없다.

로그는 최근 실행 프로세스 메모리에서 제한적으로 유지되고 작업 종료 시 저널에 저장된다. 강제 종료 직전의 모든 로그가 복원되는 기능은 아니다. 런타임이 원본 프로젝트를 삭제하면서 자동 정리하지도 않는다.

저널은 세션 512개, 작업 512개를 **상한이 아니라 보관량**으로 쓴다. 한도에 닿으면 끝난 작업 기록 중 가장 오래된 것부터 지우고 새 작업을 받는다. 세션은 마지막으로 사용한 시각이 가장 오래된 것부터 지운다. 만들어진 순서가 아니라 쓰인 순서를 기준으로 삼는 이유는, 며칠 전에 연 세션이 지금 진행 중인 대화일 수 있기 때문이다. 세션 보관량은 `CHAT2LOCAL_MAX_SESSIONS`로 조절한다.

여기에는 대가가 있다. **기록이 지워지면 그 `request_id`도 같이 풀린다.** 지워진 키를 다시 쓰면 기록된 결과를 돌려받는 대신 같은 작업이 다시 실행된다. 같은 키를 재시도하는 중에 그 기록이 밀려나는 일은 없지만, 저널이 가득 찬 상태로 재시작하면 그럴 수 있다. 그래서 `capabilities`가 `job_journal`과 `session_journal`로 보관량과 삭제 횟수를 함께 돌려준다. 이전 동작은 한도에 닿는 순간 모든 도구 호출이 실패하고, 운영자가 런타임을 멈춰 상태를 손으로 보관하기 전에는 재시작으로도 복구되지 않는 것이었다.
...
실제 프로젝트, 홈, 키체인, 브라우저 프로필, Docker 소켓에 접근할 수 없다. `.env`, `.git`, 키 파일, 링크, `node_modules`, `dist` 등은 스냅샷에서 제외된다. Docker 백엔드에서는 의존성·검증 도구가 승인한 이미지에 미리 준비되어 있어야 한다. Seatbelt 백엔드에서는 `/usr/bin` 등 시스템 경로의 호스트 도구를 그대로 볼 수 있다. 어느 쪽이든 네트워크가 없으므로 일반적인 온라인 install은 성공하지 않는다.

명령의 파일 변경과 생성물은 작업 디렉터리와 함께 사라지고 원본에 자동 적용되지 않는다. 반환되는 것은 stdout/stderr와 실행 상태다. 변경할 소스는 검토한 `write_file`로 따로 반영한다. 대용량 빌드·쓰기 공간이 큰 테스트·Docker-in-Docker는 이 제한된 워커의 지원 범위가 아니다.
