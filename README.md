...
chat2local — 세션 · 검증 · 권한 · 작업 기록
       ...
       ├─ Docker 코드 모드 → 허용된 도구를 프로그램으로 조합
       ├─ Docker 명령 실행 → 필터링된 소스 복사본, 자동 원본 반영 없음
...
Bun, Node.js 22 이상, 읽어도 되는 소스 전용 프로젝트 폴더를 준비한다. 코드 모드에는 로컬 Docker 데몬과 Node가 들어 있는 승인된 이미지가 추가로 필요하다. Aside는 브라우저 기능을 사용할 때만 필요하다. 현재 검증 대상은 Linux 런타임이며 macOS 실환경 연결은 별도 검증 항목이다. Windows 네이티브 실행은 차단되어 있으므로 Linux VM/WSL 안의 작업 폴더와 런타임을 사용한다.
...
운영자가 승인한 Node 이미지를 미리 준비하고 `CHAT2LOCAL_WORKER_IMAGE`를 설정한다. 아래 태그는 프로비저닝 예시다. 운영 배포에서는 검토한 이미지의 digest를 고정하는 편이 좋다. 런타임은 `--pull=never`를 사용하므로 이미지를 자동 다운로드하지 않는다.
