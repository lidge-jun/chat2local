...
export const schemas = {
  ...
  aside_native: z.object({ ...session, args: z.array(z.string().max(100_000)).min(1).max(32).describe('Exact Aside CLI argv, no shell. Privileged, operator opt-in required.'), request_id: request, timeout }).strict(),
...
export const descriptions: Record<ToolName, string> = {
  ...
  code_mode_read: 'Run read-only tool orchestration JavaScript in a disposable, network-disabled Docker worker. No host JS fallback. Returns job_id; poll job_get.',
  ...
  exec_command: 'Run a shell command ONLY in Docker on a filtered source snapshot. No host shell, network, secrets or live writable mount. No automatic copy-back. Returns job_id.',
