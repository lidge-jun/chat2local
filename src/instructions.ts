...
All code and shell execution run in an OS sandbox: macOS Seatbelt by default on macOS, otherwise an operator-provisioned Docker image. Read capabilities.sandbox_backend before claiming what isolates a run. Never fall back to in-process eval, vm, an unsandboxed shell, tests, install, build or typecheck.
...
Under Seatbelt the worker runs as the operator's own user with host tools on PATH; it is confined by kernel policy, not by a separate machine image. Do not describe it as a container.
Report what actually ran and what remains unverified. A configured backend is not proof that the sandbox, Aside or the ChatGPT tunnel is currently healthy.`;
